...
$(document).ready(async function(){
	...
	$("#billtable tbody tr").on("keyup change",".qty",function(key){
		...
		for(var index = 0; index < row_number; index++ )
		{
			if($("#comp"+index).find(".drawingcode").val()!=null && $("#comp"+index).find('.desc').val()!=null && $("#comp"+index).find('.qty').val()!=null && $("#comp"+index).find('.total').val()!='0.00')
			{
...
function addDispatch(dclist)
{
	...
	for(let index = 0; index < dclist.length; index++)
    {
		...
		document.getElementById("DCDate").innerHTML += "<p style='padding:0px;margin:0px;'>"+dcdate+"</p><input type='hidden' id='dcno"+index+"' name='dcdate[]' value='"+dcdate+"' style='padding:0px;' width=10/>";
	}
